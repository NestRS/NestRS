...
//! the caller's [`Ability`] and stores it on the poem request. Linking this crate
//! registers a [`ContextSeed`] that forwards that `Arc<Ability>` into every GraphQL
//! operation's context, where [`ability`] reads it back and [`authorize`](crate::authorize)
//! / [`bind`](crate::bind) gate on it.
