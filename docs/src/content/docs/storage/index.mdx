...
```rust title="crates/features/src/file_assets/module.rs"
...
```rust title="crates/features/src/file_assets/service.rs"
use std::sync::Arc;
...
`object_store`'s `ObjectMeta` does not carry it, so `HeadInfo` exposes
...
| `head(key) -> Option<HeadInfo>` | object size; `None` if absent |
...
- `crates/nest-rs-storage/` — `Storage`, `HeadInfo`, `StorageConfig`,
  `StorageModule`.
